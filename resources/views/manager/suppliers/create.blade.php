...
<div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6 max-w-3xl">
    ...
    <form method="POST" action="{{ route('suppliers.store') }}" x-data="{ submitting: false }" @submit="submitting = true">
        @csrf
        ...
        <div class="mt-6 flex gap-2">
            <button type="submit" :disabled="submitting"
                ...
                <span x-text="submitting ? 'Saving...' : 'Create Supplier'">Create Supplier</span>
            </button>
            <a href="{{ route('suppliers.index') }}" class="px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50 min-h-[40px] inline-flex items-center">Cancel</a>
        </div>
    </form>
</div>
@endsection
