...
<div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6 max-w-3xl">
    <h1 class="text-xl font-semibold text-gray-900 mb-1">Add Layer</h1>
    <p class="text-sm text-gray-500 mb-4">Layup: <span class="font-medium text-gray-700">{{ $layup->name }}</span></p>
    <form method="POST" action="{{ route('layers.store', $layup) }}" x-data="{ submitting: false }" @submit="submitting = true">
        @csrf
        ...
        <div class="mt-6 flex gap-2">
            <button type="submit" :disabled="submitting"
                ...
                <span x-text="submitting ? 'Saving...' : 'Create Layer'">Create Layer</span>
            </button>
